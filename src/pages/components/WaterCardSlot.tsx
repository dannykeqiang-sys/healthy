...
import { Trash2, Check, X, Plus, Sparkles, Loader2 } from 'lucide-react';
...
export default function WaterCardSlot({
  ...
  return (
    <div
      ...
      style={{
        background: 'linear-gradient(145deg, #EFF9FF, #E0F4FD, #F0F9FF)',
    ...
    >
      <div
        ...
        style={{ backgroundColor: config.accent, opacity: 0.14 }}
      />
      <div
      ...
      <div className="relative z-10 p-6 pb-3">
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="text-[10px] text-muted-foreground tracking-[0.18em] uppercase mt-1.5">{config.en}</p>
          </div>
          <div className="text-right pt-1">
            <p className="text-5xl font-black leading-none select-none" style={{ color: config.accent, opacity: 0.12 }}>{config.num}</p>
            <p className="text-sm font-bold mt-1 tabular-nums" style={{ color: config.accent }}>{total} ml</p>
          </div>
        </div>

        <div className="mt-3 space-y-1">
          <div className="flex justify-between text-[11px] text-muted-foreground">
            <span>{percent >= 100 ? '今日目标达成！' : `目标 ${GOAL} ml`}</span>
            <span>{percent}%</span>
          </div>
      ...
      {inputStatus === 'confirm' && (
        <div
          className="relative z-10 mx-6 mb-2 rounded-2xl border p-3 space-y-2.5"
      ...
      {inputStatus === 'error' && (
        <div className="relative z-10 mx-6 mb-2 px-3 py-2 rounded-xl text-xs text-muted-foreground border border-destructive/20 bg-destructive/5">
          {errorMsg}
        </div>
      )}
